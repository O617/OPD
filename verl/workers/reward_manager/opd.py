...
# ---- Lightweight chunk-cache dump (for P0-1 credit-rule ablation) ----
# OPD_CHUNK_CACHE_DIR: directory to persist the concatenated teacher-logps /
#   chunk-ids tensor so semantic vs. uniform runs can verify they consumed the
#   SAME upstream signal (paired-diff sanity). Disabled if unset/empty.
# OPD_CHUNK_CACHE_MAX_STEPS: stop dumping after this many training steps
#   (default: 20). Each dump is ~150-300 MB; 20 steps ≈ 4-6 GB per run.
OPD_CHUNK_CACHE_DIR = os.environ.get("OPD_CHUNK_CACHE_DIR", "")
OPD_CHUNK_CACHE_MAX_STEPS = int(os.environ.get("OPD_CHUNK_CACHE_MAX_STEPS", "20"))
_chunk_cache_step_counter = 0
...
def _dump_chunk_cache(concat_tensor):
    """Persist the [teacher_logps | chunk_ids] concat tensor for the first
    OPD_CHUNK_CACHE_MAX_STEPS training steps. Skipped when the env var is unset.

    The tensor is exactly what compute_policy_loss_opd will see as `advantages`
    on the actor side, so storing it lets us prove paired runs (semantic vs
    uniform) consumed byte-identical upstream signals.
    """
    global _chunk_cache_step_counter
    if not OPD_CHUNK_CACHE_DIR:
        return
    if _chunk_cache_step_counter >= OPD_CHUNK_CACHE_MAX_STEPS:
        return
    try:
        os.makedirs(OPD_CHUNK_CACHE_DIR, exist_ok=True)
        step_idx = _chunk_cache_step_counter
        _chunk_cache_step_counter += 1
        filepath = os.path.join(OPD_CHUNK_CACHE_DIR, f"step_{step_idx:04d}.pt")
        # Save on CPU as fp16 for teacher logps, int16 for chunk_ids to keep files small.
        T = concat_tensor.detach().cpu()
        split = T.shape[-1] // 2
        logps = T[..., :split].to(torch.float16)
        chunks = T[..., split:].round().to(torch.int16)
        torch.save({"teacher_logps": logps, "chunk_ids": chunks}, filepath)
    except Exception as e:
        warnings.warn(f"[opd chunk cache] dump failed at step {_chunk_cache_step_counter}: {e}")
...
class TeacherClient:
    ...
    def get_teacher_knowledge(self, batch: DataProto, is_async=False, student_tokenizer=None):
        ...
        def handle_futures(_bs=batch_size, _mbs=micro_batch_size):
            # NOTE: outer-scope loop bounds are captured as default-argument
            # values, NOT via closure, because `handle_futures` reassigns a
            # local `batch_size = attention_mask.size(0)` further down. Under
            # CPython's binding rules that reassignment makes `batch_size`
            # local to the entire function, so any earlier read of
            # `batch_size` would raise UnboundLocalError before we ever reach
            # the retry-branch resubmit.
            MAX_RETRIES = 3
            retry_count = 0
            while retry_count <= MAX_RETRIES:
                try:
                    ...
                    responses.clear()
                    futures.clear()
                    for i in range(0, _bs, _mbs):
                        fut = self.submit(input_ids[i : i + _mbs])
                        fut.add_done_callback(cb)
                        futures.append(fut)
