...
class VLLMEngine:
    ...
    def get_topk_logprobs(self, prompt_token_ids, temperature=0.8, max_new_tokens=1, only_response=False):
        ...
        prompts = [TokenInputs(prompt_token_ids=prompt_token_ids[i]) for i in range(len(prompt_token_ids))]
        ...
        for output in outputs:
            ...
            if self.n_logprobs > 0:
                ...
                if only_response:
                    ...
                    prompt_topk_logprobs = output.prompt_logprobs[1].logprobs[:, :].to(torch.float32)
                    prompt_topk_indices = output.prompt_logprobs[1].logprob_token_ids[:, :].to(torch.int32)
